...
function addDepartment (department_name) {
  connection.query(`INSERT INTO employee_cms.department (name) VALUES ( ? )`, department_name, function (err, results, fields) {
      if (err) {
        console.log(err);
        return;
...
function addRole(name, department, salary) {
  connection.query(`INSERT INTO employee_cms.role (title, department_id, salary) VALUES ( ? , ? ,?);`, [name, department,salary], function (err, results, fields) {
    if (err) {
      console.log(err);
      return;
    } console.log(`Added role ${name} with a salary of ${salary} to the ${department} department`)
})}


  //Add a new employee
function addEmployee (first_name, last_name,role_id, manager_id){
  connection.query(`INSERT INTO employee_cms.employee (first_name, last_name, role_id, manager_id) VALUES (?,?,?,?);`, [first_name,last_name,role_id, manager_id], function (err, results, fields) {
    if (err) {
      console.log(err);
      return;
    } console.log(`Added new employee: ${first_name} ${last_name} with a role ID of ${role_id} with a manager of ${manager_id}`)
})}
